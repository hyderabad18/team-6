...
<html>
<head><title>Unregister Event</title></head>
<body>
...
$eventid = $querystringvals["eventid"];
  ...
  if($len == 0 )
  {
    echo "Cannot unregister to this event as event id is missing";
    die();
  }
...
if ($result->num_rows == 0) {
    echo "Cannot unregister to this event as event id is invalid";
    die();
}

$sql = "SELECT id FROM EventUser WHERE eventid =" .$eventid ." and userid =" .$userid;
$result = $conn->query($sql);


if ($result->num_rows == 0) {
    echo "Cannot unregister to this event as you havent registered to it";
    die();
}
